...
def get_all_hashbands(infiles, cache_location, hasher, strip_diacritics,
    ...
    parallel_map(get_file_hashbands, buff, hasher=hasher, strip_diacritics=strip_diacritics, display=display,
                 ...
                 hashband_length=hashband_length, hashband_step=hashband_step, write_hashbands_fun=write_hashbands_fun)


def get_file_hashbands(args, hasher, strip_diacritics, display, window_length, slide_length, chargram_length,
                       hashband_length, hashband_step, write_hashbands_fun):
    """Minhash a file and save [[hashband, file_idx, window_idx]]"""
    file_idx, file_path, minhash_path = args
    minhashes = get_file_minhashes(file_path, minhash_path, hasher, strip_diacritics, display, window_length,
...
def get_file_minhashes(file_path, minhash_path, hasher, strip_diacritics, display, window_length, slide_length,
                       chargram_length):
