...
def get_all_hashbands(infiles, cache_location, hasher, strip_diacritics,
    ...
    parallel_map(get_file_hashbands, buff, cache_location=cache_location,
                 hasher=hasher, strip_diacritics=strip_diacritics, display=display, window_length=window_length,
...
def get_file_hashbands(args, cache_location, hasher, strip_diacritics, display, window_length, slide_length,
                       chargram_length, hashband_length, hashband_step, write_hashbands_fun):
    """Minhash a file and save [[hashband, file_idx, window_idx]]"""
    file_idx, file_path = args
    minhashes = get_file_minhashes(file_path, cache_location, hasher, strip_diacritics, display, window_length,
                                   slide_length, chargram_length)
...
def get_file_minhashes(file_path, cache_location, hasher, strip_diacritics, display, window_length, slide_length,
                       chargram_length):
    ...
    buff = []
    for window_idx, window in enumerate(get_windows(file_path, strip_diacritics, display, window_length, slide_length)):
